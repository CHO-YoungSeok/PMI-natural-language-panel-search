...
def bm25_search(query: str, top_k: int = 100) -> List[str]:
    """
    wellcome1st_json_data 테이블에서 BM25 알고리즘 기반 텍스트 검색

    Prerequisites:
        - ParadeDB extension이 설치되어 있어야 합니다
        - BM25 인덱스가 생성되어 있어야 합니다

    Setup commands:
        CREATE EXTENSION IF NOT EXISTS paradedb;

        CALL paradedb.create_bm25(
            index_name => 'wellcome_bm25_idx',
            table_name => 'wellcome1st_json_data',
            key_field => 'id',
            text_fields => '{"info_text": {}}'
        );

    Args:
        query: 검색 쿼리 문자열
        top_k: 반환할 최대 결과 개수 (기본값: 100)
    ...
    try:
        # ParadeDB의 BM25 검색 쿼리
        # @@@ 연산자는 ParadeDB의 full-text search 연산자
        sql = """
        ...
        # ParadeDB가 설치되지 않았거나 인덱스가 없는 경우 FTS로 fallback
        print(f"BM25 search failed: {e}")
        print("Falling back to FTS search...")
