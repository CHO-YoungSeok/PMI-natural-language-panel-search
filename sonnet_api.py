...
4. 사용자의 모든 요구조건을 빠짐없이 모두 담아야한다.
5. **사용자 입력에 ‘비흡연’, ‘비정상’, ‘非흡연’, ‘non-smoking’, ‘무면허’ 등과 같이 ‘비/非/non/무/불/미 + 단어’ 형태의 표현이 등장하면, 이를 자동으로 ‘흡연을 하지 않는’, ‘정상이 아닌’, ‘면허가 없는’처럼 뒤에 오는 단어의 의미를 부정·반대·결여·제외하는 자연스러운 한국어 문장 또는 구로 풀어서 재표현하라.```
6. 젊은층은 출생년도가 1995년도 이후, 노년층은 출생년도가 1965년 이전으로 해석해서 표현할 것.
